...
use crate::state::Message;
  ...
  fn query_messages(deps: Deps, info: MessageInfo, user2: Addr) -> Vec<Message> {
  ...
  fn self_not_in_chattable_users() {
    ...
    fn message_send_works() {
      ...
      let messages: Vec<Message> = query_messages(deps.as_ref(), user1_info.clone(), user2_info.clone().sender);
